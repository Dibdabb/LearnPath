...
import { fallbackQuestions, lessonQuestions } from './data/questions';
...
export default function App() {
  ...
  const questionsForLesson = useMemo(() => {
    if (!selectedLesson?.id) return fallbackQuestions;
    return lessonQuestions[selectedLesson.id] || fallbackQuestions;
  }, [selectedLesson?.id]);

  const currentQuestion = questionsForLesson[questionIndex % questionsForLesson.length];

  const handleAnswerQuestion = (_question, _value, isCorrect) => {
    setLastQuestionResult(isCorrect ? 'correct' : 'wrong');
    if (isCorrect) {
      setCorrectStreak((prev) => {
        const next = prev + 1;
        if (next > 0 && next % 5 === 0) {
          setShowHighFive(true);
          setTimeout(() => setShowHighFive(false), 2600);
        }
        return next;
      });
    } else {
    ...
    setQuestionIndex((prev) => (prev + 1) % questionsForLesson.length);
  };
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="bg-slate-800/70 border border-slate-700 rounded-2xl p-4 flex flex-col gap-3">
              ...
              <div className="grid md:grid-cols-[1.2fr_1fr] gap-3">
                <div className="space-y-2">
                  <p className="text-xs uppercase text-amber-200/80">Quick practice</p>
                ...
                <div className="bg-slate-900/60 border border-slate-700 rounded-xl p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs text-slate-400">Correct streak</p>
                      <p className="text-2xl font-bold text-amber-300">{correctStreak}</p>
                    </div>
                    <WolfMascot size={72} mood={lastQuestionResult === 'wrong' ? 'alert' : 'happy'} />
                  </div>
                  <p className="text-xs text-slate-300">
                    {lastQuestionResult === 'wrong'
                      ? 'Streak reset — keep going to reach 5 in a row!'
                      : 'Hit 5 correct answers in a row and the wolf will high-five you.'}
                  </p>
                </div>
              </div>
            </div>
      ...
      {showHighFive && <HighFiveOverlay streak={correctStreak} />}
    </div>
  );
}
...
function HighFiveOverlay({ streak }) {
  return (
    <div className="fixed inset-0 pointer-events-none flex items-end justify-center pb-10">
      <div className="animate-bounce bg-slate-900/90 border border-amber-300/40 rounded-2xl px-5 py-4 flex items-center gap-3 shadow-lg">
        <WolfMascot size={88} />
        <div className="space-y-1">
          <p className="text-sm text-amber-200 uppercase tracking-wide">High five!</p>
          <p className="text-lg font-semibold text-white">{`That's ${streak} correct answers in a row!`}</p>
          <p className="text-xs text-slate-300">The golden wolf is celebrating your streak — keep it going.</p>
        </div>
      </div>
    </div>
  );
